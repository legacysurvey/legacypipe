...
for chunk in range(args.istart,args.istop+1): 
    ...
    ap('export DECALS_SIM_DIR=/project/projectdirs/desi/users/burleigh/decals_sim/durham/2523p355_nosimnoise_r18')
    ...
    ap('srun -n 1 -c ${OMP_NUM_THREADS} python legacyanalysis/decals_sim.py --brick %s -n 500 -o STAR -ic %d --rmag-range 18 18 --threads ${OMP_NUM_THREADS}' % (brick,chunk))
    ap('echo DONE')
    fin.close()
    #ap('sbatch %s' % fn) 
