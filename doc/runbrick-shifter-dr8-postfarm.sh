...
ncores=8
if [ -n "$(grep avx512 /proc/cpuinfo)" ]; then
    # 64 GB / Edison node = 67108864 kbytes
    # 96 GB / KNL node >= 95839232 KB
    maxmem=88839232
...
python -O legacypipe/runbrick.py \
     ...
     --skip-calibs \
     --threads ${ncores} \
     --checkpoint "/global/cscratch1/sd/ziyaoz/farm-checkpoints-prod-pt4/checkpoint-${1}.pickle" \
     --pickle "/global/cscratch1/sd/landriau/dr8/decam-des/pickles/%(brick).3s/runbrick-%(brick)s-%%(stage)s.pickle" \
     --unwise-coadds \
     --outdir $outdir \
