...
def main():
    ...
    from legacypipe.unit import get_units_for_columns
    ...
    for b in bands:
        ...
        T.set('forced_apflux_%s' % b, np.zeros(len(T), np.float32))
        T.set('forced_apflux_ivar_%s' % b, np.zeros(len(T), np.float32))
