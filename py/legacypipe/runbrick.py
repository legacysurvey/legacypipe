...
def _one_blob(X):
    ...
    ok,x0,y0 = subtarget.radec2pixelxy(
    ...
    for (subimg, subie, twcs, subwcs, pcal,
        ...
        subsubwcs = subwcs.get_subimage(int(sx0), int(sy0),
    ...
    if len(subcat) > 1:
        ...
        for numi,i in enumerate(Ibright):
            ...
            if plots and False:
                tims_compute_resamp(None, srctractor.getImages(), targetwcs)
                tims_compute_resamp(None, subtims, targetwcs)
    ...
    for numi,i in enumerate(Ibright):
        ...
        if bigblob:
            ...
            for tim in srctims:
                try:
                    Yo,Xo,Yi,Xi,nil = resample_with_wcs(subtarget, tim.subwcs,
                                                        [],2)
                except:
                    continue
        ...
        if plots:
            tims_compute_resamp(None, srctims, subtarget)
            plt.clf()
            ...
            for tim in srctims:
                del tim.resamp
            tims_compute_resamp(None, srctims, srctarget)
            plt.clf()
            coimgs,cons = compute_coadds(srctims, bands, srctarget,
                                         fill_holes=False)
            dimshow(get_rgb(coimgs, bands))
            plt.title('Model selection: stage1 data')
            ps.savefig()

            # srch,srcw = srctarget.shape
        ...
        for name,newsrc in trymodels:
            ...
            if plots:
                ...
                comods,nil = compute_coadds(srctims, bands, srctarget,
                                            images=modimgs)
                dimshow(get_rgb(comods, bands))
            ...
            if hastycho:
                # print('There is a Tycho-2 star in this blob; not doing second-round optimization')
                modtims = None
            elif bigblob:
                mods = []
                for tim in subtims:
                    ...
                    if mod is not None:
                        ...
                        if mod.patch is None:
                            mod = None
                    mods.append(mod)
                modtims,mm = _get_subimages(subtims, mods, newsrc)
            else:
                mm = []
                modtims = []
                for tim in subtims:
            ...
            if modtims is not None:
                ...
                if plots:
                    ...
                    tims_compute_resamp(None, modtims, srctarget)
                    comods,nil = compute_coadds(modtims, bands, srctarget,
                                                images=modimgs)
                    dimshow(get_rgb(comods, bands))
        ...
        if plots:
            ...
            for imod,modname in enumerate(mods.keys()):
                ...
                if modname != 'none':
                    ...
                    dimshow(get_rgb(coimgs, bands))
                    ax = plt.axis()
                    ok,x,y = subtarget.radec2pixelxy(src.getPosition().ra,
                                                     src.getPosition().dec)
                    plt.plot(x-1, y-1, 'r+')
                    plt.axis(ax)
                    plt.title('Image')
                ...
                if False:
                    cochisqs,nil = compute_coadds(subtims, bands, subtarget,
                                                  images=chis)
                    ...
                    # residuals
                    coresids,nil = compute_coadds(srctims, bands, srctarget,
                                                  images=res)
                    plt.subplot(rows, cols, imod+1+cols)
    ...
    if plots:
        ...
        _plot_mods(subtims, plotmods, plotmodnames, bands, None, None, bslc,
                   blobw, blobh, ps)
    ...
    ok,x1,y1 = subtarget.radec2pixelxy(
...
def _get_subimages(tims, mods, src):
    ...
    for tim,mod in zip(tims, mods):
        ...
        if subimg.shape != (mh,mw):
            print('Subimage shape:', subimg.shape, 'image shape', tim.getImage().shape, 'slice y', y0,y1, 'x', x0,x1, 'mod shape', mh,mw)
...
def main():
    ...
    import datetime
    de = 'Main "pipeline" script for the Dark Energy Camera Legacy Survey (DECaLS) data reductions.'
...
'''
    ...
    parser.add_argument('--blobxy', type=int, nargs=2, default=None, action='append',
                      help='Debugging: run the single blob containing pixel <bx> <by>; this option can be repeated to run multiple blobs.')

    parser.add_argument('--no-pv', dest='pv', default='True', action='store_false',
    ...
    parser.add_argument('--check-done', default=False, action='store_true',
    ...
    parser.add_argument('--skip-calibs', dest='do_calibs', default=True,
    ...
    parser.add_argument('--nsigma', type=float, help='Set N sigma source detection thresh')

    parser.add_argument('--simul-opt', action='store_true', default=False,
    ...
    parser.add_argument('--unwise-dir', default=None,
    ...
    parser.add_argument('--pixpsf', action='store_true', default=False,
    ...
    parser.add_argument('--coadd-bw', action='store_true', default=False,
    ...
    parser.add_argument('--no-blacklist', dest='blacklist', default=True,
                      action='store_false', help='Do not blacklist some proposals?')

    parser.add_argument('--rsync', default=False, action='store_true',
