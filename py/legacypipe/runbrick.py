...
def stage_overlap_sky(tims=None, bands=None, targetwcs=None,
                      blobs=None, plots=False, ps=None, mp=None, **kwargs):
    #from collections import Counter
    #print('Blob map:', Counter(blobs.ravel()).most_common())
    ...
    for band,co,cowt in zip(bands, C.coimgs, C.cowimgs):
        ...
        if len(pix) == 0:
            continue
        if plots:
            ha = dict(bins=50, range=(-0.01, 0.01))
            plt.clf()
            plt.hist(pix, **ha)
            plt.axvline(np.median(pix), lw=3, color='k')
            plt.title('%s band, masked pixels' % band)
            ps.savefig()

        print('Median of coadd:', np.median(pix))
        from astrometry.util.miscutils import estimate_mode
        print('Mode of coadd:', estimate_mode(pix))

        #cosky = np.median(pix)
        cosky = estimate_mode(pix)
        ...
        for tim in tims:
            ...
            try:
                ...
                continue
            print(len(Yo), 'pixels of', tim.name, 'overlap brick')
            good = (cowt[Yo,Xo] > 0) * (blobs[Yo,Xo] == -1) * (tim.getInvError()[Yi,Xi] > 0)
            print(np.sum(good), 'pixels are good')
            imgpix = tim.getImage()[Yi[good],Xi[good]]
            copix = co[Yo[good], Xo[good]]
            print('  diff of medians:', np.median(imgpix) - np.median(copix))
            print('  median of diffs:', np.median(imgpix - copix))
            #print('  image median', np.median(imgpix))
            #print('  coadd median', np.median(copix))
            #print('  diff median', np.median(imgpix - copix))

            if plots:
                plt.hist(imgpix.ravel(), histtype='step', **ha)
                plt.axvline(np.median(imgpix), lw=2, alpha=0.5, color='g')

            dmeds.append(np.median(imgpix) - np.median(copix))
            mco.append(np.median(copix))
            mdiffs.append(np.median(imgpix - copix))
            mpix.append(np.median(imgpix))
            mopix.append(estimate_mode(imgpix))
            sigs.append(tim.sig1)
                
            #correction = -cosky + (np.median(copix) - np.median(imgpix)) #np.median(copix - imgpix)
            correction = -cosky
            tim.data += correction
        ...
        if plots:
            ...
            plt.clf()
            plt.plot(sigs, dmeds, 'k.')
            plt.xlabel('sig1')
            plt.title('%s band: med(img) - med(co)' % band)
            ps.savefig()

            plt.clf()
            plt.plot(sigs, mdiffs, 'k.')
            plt.xlabel('sig1')
            plt.title('%s band: med(img - co)' % band)
            ps.savefig()

            plt.clf()
            plt.plot(sigs, mpix, 'k.')
            plt.xlabel('sig1')
            plt.title('%s band: med(img)' % band)
            ps.savefig()

            plt.clf()
            plt.plot(sigs, mopix, 'k.')
            plt.xlabel('sig1')
            plt.title('%s band: mode(img)' % band)
            ps.savefig()

            plt.clf()
            plt.plot(sigs, mco, 'k.')
            plt.xlabel('sig1')
            plt.title('%s band: med(co)' % band)
            ps.savefig()

    print('Re-creating coadd...')
    C = make_coadds(tims, bands, targetwcs, mp=mp, sbscale=False)
    for band,co,cowt in zip(bands, C.coimgs, C.cowimgs):
        print(band, 'band')
        pix = co[(cowt > 0) * (blobs == -1)]
